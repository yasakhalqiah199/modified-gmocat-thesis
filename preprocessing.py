...
def featurize_eedi(dataset):
    ...
    TRAIN_DATA = os.path.join(script_dir, 'raw_data', 'train_task_'+dataset+'.csv')
    ANSWER_DATA = os.path.join(script_dir, 'raw_data', 'answer_metadata_task_'+dataset+'.csv')

    if not os.path.exists(TRAIN_DATA) or not os.path.exists(ANSWER_DATA):
        print(f"Error: EEDI dataset files not found. Expected: {TRAIN_DATA}, {ANSWER_DATA}")
        return
    ...
    k2n = {}
    with open(os.path.join(script_dir, 'raw_data', 'question_metadata_task_'+dataset+'.csv'), 'r') as fp:
    ...
    dump_json(os.path.join(script_dir, 'data', f'train_task_{dataset}.json'), results)
    dump_json(os.path.join(script_dir, 'data', f'question_map_{dataset}.json'), question_map)
    dump_json(os.path.join(script_dir, 'data', f'concept_map_{dataset}.json'), q2k)
...
def featurize_junyi():
    global question_map, df
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_path = os.path.join(script_dir, 'raw_data', 'junyi_ProblemLog_for_PSLC.txt')
    if not os.path.exists(data_path):
        print(f"Error: Junyi dataset file not found at {data_path}")
        return
    ...
    dump_json(os.path.join(script_dir, 'data', 'train_task_junyi.json'), results)
    dump_json(os.path.join(script_dir, 'data', 'question_map_junyi.json'), question_map) 
    dump_json(os.path.join(script_dir, 'data', 'concept_map_junyi.json'), q2k)
...
def featurize_assist2009():
    global question_map, df
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_path = os.path.join(script_dir, 'raw_data', 'assist09.csv')
    if not os.path.exists(data_path):
        print(f"Error: Assistments 2009 dataset file not found at {data_path}")
        print("Please download the dataset and place it in the 'raw_data' directory, or skip this step if using pre-processed 'dbekt22' data.")
        return

    df = pd.read_csv(data_path, encoding = 'ISO-8859-1', dtype={'skill_id': str}, low_memory=False,
    ...
    dump_json(os.path.join(script_dir, 'data', 'train_task_assist2009.json'), results)
    dump_json(os.path.join(script_dir, 'data', 'question_map_assist2009.json'), question_map)
    dump_json(os.path.join(script_dir, 'data', 'concept_map_assist2009.json'), q2k)
