...
class ActorCritic(nn.Module):
    ...
    def hidden_layer(self, p_rec, p_target, a_rec, kn_rec, kn_num):# kn_rec (B,L,num_k)
        if self.args.use_graph:
            ...
            # Align kn_rec last-dim with kn_emb2 rows (graph may imply extra padding)
            target_k = kn_emb2.size(0)
            if kn_rec.size(-1) < target_k:
                pad_size = target_k - kn_rec.size(-1)
                pad = torch.zeros((*kn_rec.shape[:-1], pad_size), device=kn_rec.device, dtype=kn_rec.dtype)
                kn_rec_aligned = torch.cat([kn_rec, pad], dim=-1)
            elif kn_rec.size(-1) > target_k:
                kn_rec_aligned = kn_rec[:, :, :target_k]
            else:
                kn_rec_aligned = kn_rec

            batch_kn_emb = torch.matmul(kn_rec_aligned, kn_emb2) / kn_num.unsqueeze(-1) # B L D
        else:
            # get batch exercise data
